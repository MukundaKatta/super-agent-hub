// super-agent-hub — registry module
...
export class Registry extends EventEmitter {
  ...
  constructor(options: RegistryOptions = {}) {
    super();
    this.options = { name: options.name ?? "Registry", enabled: options.enabled ?? true, timeout: options.timeout ?? 30000 };
  }
  ...
  async process(input: unknown): Promise<RegistryResult> {
